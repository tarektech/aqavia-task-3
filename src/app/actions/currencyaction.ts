...
import { FALLBACK_CURRENCIES, FALLBACK_RATES, REQUIRED_CURRENCIES, type ApiResponseType, type ConversionResultType } from "@/types/types";
...
export async function getCurrenciesSymbols(): Promise<string[]> {
    try {
        ...
        const prioritized = REQUIRED_CURRENCIES.filter(c => currencies.includes(c));
...
function convertWithFallback(from: string, to: string, amount: number): ConversionResultType {
